...
public class StoplossService implements StoplossListener {
	...
	@Setter private AccountService accountService;
	@Setter private ExchangeService exchangeService;
	@Setter private StoplossWatcher watcher;
	
	private List<StoplossOrder> orders;

	@Autowired
	public StoplossService(AccountService accountService, ExchangeService exchangeService, StoplossWatcher watcher, OrderPlacer orderPlacer) {
	...
	private void checkAltCoinBalances(String exchange, Account account) {
		try {
			...
			while (balanceIterator.hasNext()) {
				Entry<String, Double> currency = balanceIterator.next();
	...
	private void processBalance(String exchange, Account account, Entry<String, Double> balance) {
	...
	private void processAltcoin(String exchange, Account account, Entry<String, Double> balance) {
	...
	public void checkStoploss() {
		...
		orders.removeIf(this::stoplossTriggered);
	}
